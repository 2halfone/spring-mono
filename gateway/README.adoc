...
The Dockerfile for this gateway service is located in the current directory: `Dockerfile`.

=== Configuration

The gateway service is configured using Spring profiles and can be influenced by environment variables.

*Environment Variables:*
* `SPRING_PROFILES_ACTIVE`: Sets the active Spring profile (e.g., `staging`, `prod`). This determines which `application-{profile}.properties` file is loaded from `initial/src/main/resources/`.

*Spring Profiles:*
Profile-specific property files (e.g., `application-staging.properties`, `application-prod.properties`) are located in `initial/src/main/resources/`. These files define configurations such as the server port and Spring Cloud Gateway routes. For example:
...
*   Internally, the gateway service runs on port `8080` (as configured by `server.port` in its application properties within the `initial` subdirectory).
*   When running via Docker (e.g., using the project-level `docker-compose.staging.yml`), this internal port `8080` is typically mapped to a different host port (e.g., `8083:8080`).
